...
static const uint32_t eepromSizes[] = {16384, 8192, 4096, 2048,
                                       1024,  512,  256,  0};
#if defined(__SAMD51__)
#define EEPROM_EMULATION_RESERVATION 0
#else
#define EEPROM_EMULATION_RESERVATION               \
  ((*(uint32_t*)(NVMCTRL_FUSES_EEPROM_SIZE_ADDR) & \
    NVMCTRL_FUSES_EEPROM_SIZE_Msk) >>              \
   NVMCTRL_FUSES_EEPROM_SIZE_Pos)
#endif
extern "C" {
char* __text_start__();  // 0x2000, 0x0 without bootloader and 0x4000 for M0
                         // original bootloader
}
...
extern "C" char*
    g_pfnVectors;  // at first address of the binary. 0x0000 without bootloader.
                   // 0x2000 with Maple bootloader
...
OTAStorage::OTAStorage()
    :
      ...
      MAX_FLASH(PAGE_SIZE * NVMCTRL->PARAM.bit.NVMP -
                eepromSizes[EEPROM_EMULATION_RESERVATION])
#elif defined(ARDUINO_ARCH_NRF5)
      ...
      SKETCH_START_ADDRESS(
          ...
          FLASH_BASE),  // start address depends on bootloader size
...
#endif
      MAX_FLASH(
          (min(LL_GetFlashSize(), 512ul) *
           1024))  // LL_GetFlashSize returns size in kB. 512 is max for a bank
